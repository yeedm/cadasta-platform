...
class LocationAddTest(ViewTestCase, UserTestCase, TestCase):
    ...
    # HACK: Workaround for django-skivvy not supporting META manipulation
    def setup_view(self, view_kwargs={}):
        if hasattr(self, 'http_referer'):
            self._request.META['HTTP_REFERER'] = self.http_referer
        return super().setup_view(view_kwargs=view_kwargs)

    def test_get_with_authorized_user_with_referrer(self):
        user = UserFactory.create()
        assign_policies(user)
        self.http_referer = '/help/'
        response = self.request(user=user)
        assert response.status_code == 200
